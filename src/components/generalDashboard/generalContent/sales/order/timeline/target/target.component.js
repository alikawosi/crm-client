...
class Target extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            invoiceColumnDefs: [
                ...
                { headerName: 'کد', field: "code", valueGetter: this.codeValueGetter },
                { headerName: 'کد مرجع', field: "referenceCode", valueGetter: this.referenceCodeValueGetter },
                { headerName: 'تاریخ فاکتور', field: "nativeDate" },
                { headerName: 'خریدار', field: "buyer" },
                { headerName: 'فروشنده', field: "seller" },
                { headerName: 'مبلغ کل فاکتور', field: "price", valueFormatter: this.currencyFormatter, },
                { headerName: 'مبلغ کل اضافات', field: "additions", valueFormatter: this.currencyFormatter, },
                { headerName: 'مبلغ کل کسورات', field: "deductions", valueFormatter: this.currencyFormatter, },
                { headerName: 'مبلغ قابل پرداخت', field: "payablePrice", valueFormatter: this.currencyFormatter, },
                { headerName: 'تاریخ ثبت', field: "nativeDateCreated" },
    ...
    componentDidMount() {
        this.setColumnDefsValue();
    }
    /* #endregion */

      /* #region  [- setColumnDefsValue -] */
      setColumnDefsValue() {
        if (this.props.timelineTargetList[0].quoteFlag) {     
           return   this.state.quoteColumnDefs    
        }
        else if (this.props.timelineTargetList[0].orderFlag) {
            return   this.state.orderColumnDefs   
        }
        else if (this.props.timelineTargetList[0].invoiceFlag) {
            return   this.state.invoiceColumnDefs   
        }
    }
    /* #endregion */
                ...
                <Row name="row_02_Grid">
                    ...
                    <Col sm="12" md="12" lg="12" className="ag-theme-alpine mt-2" style={{ height: '70vh', width: '100%', marginTop: '2%' }}>
                        <AgGridReact
                            ...
                            columnDefs={this.setColumnDefsValue()}
                            rowData={this.props.timelineTargetList}
                            enableRtl={true}
...
const mapStateToProps = state => {
    return {
        ...
        timelineTargetList: state.orderTimeline.timelineTargetList,
