...
import {
    saveOrderRegisteredDocumentsProductData, setSaveButtonDisabled,
    ...
    onRowSelected = async (event) => {
        if (this.state.counter === 0) {
            ...
            if (event.node.selected === true) {
                ...
                await this.props.saveOrderRegisteredDocumentsProductData(newList);
...
const mapStateToProps = state => {
    return {
        ...
        registeredDocumentsItemProductList: state.order.registeredDocumentsItemProductList,
        registeredDocumentsType: state.order.registeredDocumentsType,
    }
};
/* #endregion */
