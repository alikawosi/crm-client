...
class GridDeductionsButton extends PureComponent {
    ...
    gridLoadData = () => {
        if (this.props.sumAllDeductions.length !== 0) {
            ...
            let list = this.props.orderItemProductDeductionsList.filter(x => x.productRef === this.props.node.data.id)
            let sumDeduction = 0
            list.map(x => {
            ...
            let rowCount = this.props.orderItemProductDeductionsList.filter(x => x.productRef === this.props.node.data.id).length
