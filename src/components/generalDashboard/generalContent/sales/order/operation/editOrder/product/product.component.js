...
import {
    getOrderPriceListProduct, getOrderProduct, editOrderProductData, saveProductTotalPrice,
    editOrderProductAdditions, editOrderProductDeductions, editOrderFinancialCaseType, getOrderInventory,
    setSaveButtonDisabled,
    saveOrderRegisteredDocumentsProductData, editProductTotalQuantity,
...
class Product extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedProductColumnDefs: [
                ...
                {
                    headerName: 'مبلغ پیش فرض', field: "unitPrice", headerComponentParams: { menuIcon: 'fa fa-pencil' }, editable: true, colId: "unitPrice",
                    valueGetter: this.unitPriceValueGetter, pinnedRowCellRenderer: 'customPinnedRowRenderer', width: 80, valueFormatter: this.currencyFormatter,
                },
                {
                ...
                { headerName: 'مبلغ', field: "price", valueFormatter: this.currencyFormatter, colId: "price", width: 80, valueGetter: this.priceValueGetter, pinnedRowCellRenderer: 'customPinnedRowRenderer', },
                {
                ...
                { headerName: 'تامین کننده', field: "supplyChainTitle", },
                { headerName: 'نام گروه کالا', field: "productCategoryTitle", },
                { headerName: 'توضیحات', field: "descriptionRow", colId: "descriptionRow", headerComponentParams: { menuIcon: 'fa fa-pencil' }, editable: true, pinnedRowCellRenderer: 'customPinnedRowRenderer', },
    ...
    async componentDidMount() {
        ...
        await this.productGridLoadData()
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.orderItemProductList !== this.props.orderItemProductList) {
    ...
    /* #region  [- productGridLoadData -] */
    productGridLoadData = async () => {
        await this.calculatePrice(this.props.orderItemProductList)
        this.setState({
            selectedProductList: this.props.orderItemProductList
        })
    ...
    calculateAllProductFinancialCase = async () => {
        ...
        if (lenSumAllAdditions === 0 && lenSumAllDeductions===0 ) {
            let list = [...this.props.orderItemProductList]
            ...
            let additionData = {
                tempList: this.props.orderItemProductAdditionsList,
                sumAllAdditions: finalSumAllAdditions
            }
            await this.props.editOrderProductAdditions(additionData);
            let deductionData = {
                tempList: this.props.orderItemProductDeductionsList,
                sumAllDeductions: finalSumAllDeductions
            }
            await this.props.editOrderProductDeductions(deductionData);
        }
    }
    /* #endregion */
    ...
    refreshOrderProductAdditionsDeductions = async (selectedProductList, productRef) => {
        let productQuantityList = [...this.props.orderItemProductQuantityList]
        let quantity = 0
        ...
        //additions
        let orderProductAdditionList = [...this.props.orderItemProductAdditionsList]
        ...
        if (additionsLen > 0) {
            ...
            this.props.editOrderProductAdditions(data);
        }
        //deductions
        let orderProductDeductionList = [...this.props.orderItemProductDeductionsList]
        ...
        if (decductionsLen > 0) {
            ...
            this.props.editOrderProductDeductions(data);
        }
    ...
    calculatePrice = async (selectedProductList) => {
        ...
        await this.props.editOrderProductData(selectedProductList);
    ...
    refreshOrderFinancialCase = async (totalPrice) => {
        let orderItemFinancialCaseList = [...this.props.orderItemFinancialCaseList]
        orderItemFinancialCaseList.map((item) => {
            if (item.percentFlag === true) {
                item.financialCasePrice = ((item.financialCasePercent * 1) * totalPrice / 100) * 1
            }
        }

        )
        await this.props.editOrderFinancialCaseType(orderItemFinancialCaseList)
    }
    /* #endregion */
    ...
    deleteProductFinancialCase = () => {
        this.state.deletedProductList.map(y => {
            let additionList = this.props.orderItemProductAdditionsList.filter(x => x.productRef !== y.id)
            let additionData = {
                tempList: additionList,
                sumAllAdditions: this.props.sumAllAdditions
            }
            this.props.editOrderProductAdditions(additionData)

            let deductionList = this.props.orderItemProductDeductionsList.filter(x => x.productRef !== y.id)
            let deductionData = {
                tempList: deductionList,
                sumAllDeductions: this.props.sumAllDeductions
            }
            this.props.editOrderProductDeductions(deductionData)
    ...
    calculateOrderProductQuantity = async () => {
        let tempList = this.props.orderItemProductQuantityList
    ...
    save = async () => {
        if (this.state.isProductGridHidden === false) {
            ...
            if (this.state.selectedProductList.length === 0) {
                ...
                await this.props.editOrderProductData(this.state.selectedProductList)
                this.setState({
                    productList: [],
                    registeredDocumentsList: [],
                    priceListRef: '',
    ...
    delete = async () => {
        ...
        await this.props.editOrderProductData(list);
...
const mapDispatchToProps = dispatch => ({
    ...
    editOrderProductData: (data) => dispatch(editOrderProductData(data)),
    saveProductTotalPrice: (data) => dispatch(saveProductTotalPrice(data)),
    editOrderProductAdditions: (data) => dispatch(editOrderProductAdditions(data)),
    editOrderProductDeductions: (data) => dispatch(editOrderProductDeductions(data)),
    editOrderFinancialCaseType: (data) => dispatch(editOrderFinancialCaseType(data)),
