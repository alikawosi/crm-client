...
class ProductQuantity extends PureComponent {
    ...
    save = async () => {
        ...
        if (requestesNumber === "" || requestesNumber === undefined || requestesNumber < 1 || isNaN(requestesNumber) === true) {
            ...
            let lenPureOrderItemProductQuantityList=Object.keys(this.props.pureOrderItemProductQuantityList.filter(y => y.inventoryId === inventoryId && y.productRef===productRef)).length
            ...
            let pureOrderItemProductRequestesNumber = 0
            let selectedProductListRequestesNumber = 0

            productInventory = this.props.orderInventoryList.filter(x => x.inventoryId === inventoryId)[0].productInventory * 1
          
            pureOrderItemProductRequestesNumber = lenPureOrderItemProductQuantityList===0?0:this.props.pureOrderItemProductQuantityList.filter(y => y.inventoryId === inventoryId && y.productRef===productRef)[0].requestesNumber * 1
            ...
            isInventoryRequestesNumberPermitted = productInventory + pureOrderItemProductRequestesNumber >= requestesNumber + selectedProductListRequestesNumber ? true : false
            ...
            if (isInventoryRequestesNumberPermitted === false) {
                ...
                if (Object.keys(newList).length > 0) {
                    ...
                    await this.gridApi.setRowData(this.props.orderInventoryList);
    ...
    /* #region  [- submit -] */
    submit = async () => {
        let tempList = this.props.orderItemProductQuantityList.filter(x => x.productRef !== this.props.productRef)
        this.state.selectedProductList.map(x => {
            tempList.push(x)
        })
...
const mapStateToProps = state => {
    return {
        ...
        orderInventoryList: state.order.orderInventoryList,
        orderItemProductQuantityList: state.order.orderItemProductQuantityList,
        pureOrderItemProductQuantityList: state.order.pureOrderItemProductQuantityList,
