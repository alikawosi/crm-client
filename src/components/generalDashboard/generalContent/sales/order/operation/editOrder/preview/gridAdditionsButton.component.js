...
import { editOrderProductAdditions } from '../../../../../../../../redux/sales/order/order/order.action'
...
class GridAdditionsButton extends PureComponent {
    ...
    gridLoadData = () => {
        if (this.props.sumAllAdditions.length !== 0) {
            ...
            let list = this.props.orderItemProductAdditionsList.filter(x => x.productRef === this.props.node.data.id)
            let sumAddtion = 0
            list.map(x => {
                if (x.percentFlag === true) {
                    x.financialCasePrice = ((x.financialCasePercent*1) * (this.props.node.data.unitPrice * this.props.node.data.quantity) / 100)*1
                }
                sumAddtion = sumAddtion + x.financialCasePrice
            })
            let finalSumAddition = Object.assign({}, sumAdditionsList[0]).sumAdditions = sumAddtion
            //let rowCount = this.props.orderItemProductAdditionsList.filter(x => x.productRef === this.props.node.data.id).length
