    ...
    gridLoadData = async () => {
        ...
        if (len > 0) {
            await this.gridApi.forEachNode((rowNode) => {
                for (let i = 0; i < len; i++) {
                    if (orderMergeProductList[i].headerId === rowNode.data.orderHeaderId &&
        ...
        if (this.state.counter === 0) {
            ...
            if (event.node.selected === true) {
                ...
                let newList = list.filter(x => {
                    if (x.productRef === data.productRef && x.headerId === data.orderHeaderId) {
                ...
                //let newList = list.filter(x => x.productRef !== data.productRef && x.headerId!==data.orderHeaderId)
