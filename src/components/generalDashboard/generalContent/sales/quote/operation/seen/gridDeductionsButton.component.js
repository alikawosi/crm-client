    ...
    /* #endregion */
     ...
     gridLoadData = () => {
        if (this.props.sumAllDeductions.length !== 0) {
            ...
            let list = this.props.quoteItemProductDeductionsList.filter(x => x.productRef === this.props.node.data.id)
            let sumDeduction = 0
            list.map(x => {
                if (x.percentFlag === true) {
                    x.financialCasePrice = parseFloat(parseInt(x.financialCasePercent) * (this.props.node.data.unitPrice * this.props.node.data.quantity) / 100)
                }
            ...
            let rowCount = this.props.quoteItemProductDeductionsList.filter(x => x.productRef === this.props.node.data.id).length
