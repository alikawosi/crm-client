...
import {
    editQuoteFinancialCaseType, postQuoteFinancialCaseType, getQuoteFinancialCaseTypeTitle, editQuoteProductAdditions,
    editQuoteProductDeductions, saveProductTotalPrice
...
class FinancialCase extends PureComponent {
    ...
    gridLoadData = async () => {
        await this.setState({
            rowData: this.props.quoteItemFinancialCaseList,
    ...
    // /* #region  [- validateFinancialCasePrice -] */
    // validateFinancialCasePrice = (e) => {
    //     let len = Object.keys(e.target.value).length

    //     if (len === 0 || len === 1) {
    //         this.setState({
    //             financialCasePrice: e.target.value,
    //             financialCasePriceDisplayName: e.target.value
    //         })
    //     }
    //     else if (len > 1) {
    //         let value = e.target.value.slice(len - 1, len + 1)
    //         let charCode = value.charCodeAt(0);
    //         if ((charCode > 47 && charCode < 58) || charCode === 46) {
    //             let financialCasePriceLen = Object.keys(this.state.financialCasePrice).len
    //             this.setState({
    //                 financialCasePrice: financialCasePriceLen === 0 ? value : this.state.financialCasePrice.concat(value),
    //                 financialCasePriceDisplayName: financialCasePriceLen === 0 ? value : this.state.financialCasePrice.concat(value).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')
    //             })
    //         }
    //     }
    //     this.validateForm(e);
    ...
    save = async () => {
        if (this.validateFormOnButtonClick() === true) {
            ...
            if (this.state.isPercentChecked === true) {
                percentOrCash = 'درصدی'
                financialCasePrice = parseFloat((this.state.financialCasePercent * 1) * this.props.productTotalPrice / 100)
            ...
            await this.props.editQuoteFinancialCaseType(this.state.rowData)
            await this.calculateQuoteTotalPrice()
            this.onPinnedRowBottomCount();
        }
    ...
    delete = async () => {
        ...
        await this.props.editQuoteFinancialCaseType(list)
        await this.calculateQuoteTotalPrice()
        this.onPinnedRowBottomCount();
    }
    /* #endregion */
    ...
    /* #region  [- handleChange -] */
    handleChange = (e) => {
        // if (e.target.name === 'financialCasePrice') {
        //     this.validateFinancialCasePrice(e);
        // }
        // else {}
        this.setState({
                    ...
                    <Col sm='12' md='12' lg='12' style={{ textAlign: 'right' }}>
                        <Form style={{ padding: '1%' }}>
                            ...
                            <Row name='useTemplateButton_AdditionOrDeduction_PercentOrCash'>
                                ...
                                {/* <Col class="d-sm-none d-md-none col-lg-6" ></Col> */}
                                <Col sm='6' md='6' lg='9' name='useTemplate'>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }} >
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <td style={{ direction: 'ltr' }}>{this.state.quoteTotalPrice.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</td>
                                </tr>
