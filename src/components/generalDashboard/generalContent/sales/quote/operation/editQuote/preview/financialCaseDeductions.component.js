...
class FinancialCaseDeductions extends PureComponent {
    ...
    createData = () => {
        var result = [];
        if (Object.keys(this.props.quoteItemFinancialCaseList).length > 0) {
            let financialCasePrice = 0
            let financialCasePercent = 0
            this.props.quoteItemFinancialCaseList.filter(y => y.deductionsFlag === true).map(x => {
    ...
    /* #region  [- gridLoadData -] */
    gridLoadData = () => {
        let list = this.props.quoteItemFinancialCaseList.filter(x => x.deductionsFlag === true)
        this.setState({
            rowData: list
        })
    }
                                ...
                                <Col sm='12' md='3' lg='3'>
                                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }} >
                                        <thead>
                                            ...
                                            <tr>
                                                <td>{this.props.sumDeductions.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')} </td>
                                            </tr>
