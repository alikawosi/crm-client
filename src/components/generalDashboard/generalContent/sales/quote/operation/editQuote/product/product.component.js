...
import {
    getQuotePriceListProduct, getQuoteProduct, editQuoteProductData, saveProductTotalPrice,
    editQuoteProductAdditions, editQuoteProductDeductions, editQuoteFinancialCaseType,
...
class Product extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedProductColumnDefs: [
                ...
                { headerName: 'تامین کننده', field: "supplyChainTitle", },
                { headerName: 'نام گروه کالا', field: "productCategoryTitle", },
                { headerName: 'توضیحات', field: "descriptionRow", colId: "descriptionRow", headerComponentParams: { menuIcon: 'fa fa-pencil' }, editable: true },
    ...
    productGridLoadData = async () => {
        ...
        await this.calculatePrice(this.props.quoteItemProductList)
        this.setState({
            selectedProductList: this.props.quoteItemProductList
        })
    ...
    calculateAllProductFinancialCase = async () => {
       ...
       if(lenSumAllAdditions===0 && lenSumAllDeductions===0){
        let list = [...this.props.quoteItemProductList]
        ...
        let additionData = {
            tempList: this.props.quoteItemProductAdditionsList,
            sumAllAdditions: finalSumAllAdditions
        }
        await this.props.editQuoteProductAdditions(additionData)
        let deductionData = {
            tempList: this.props.quoteItemProductDeductionsList,
            sumAllDeductions: finalSumAllDeductions
        }
        await this.props.editQuoteProductDeductions(deductionData)
    }
    }
    /* #endregion */
    ...
    refreshQuoteProductAdditionsDeductions = async (selectedProductList, productId) => {
        ...
        //additions
        let quoteProductAdditionList = [...this.props.quoteItemProductAdditionsList]
        ...
        if (additionsLen > 0) {
            ...
            this.props.editQuoteProductAdditions(data);
        }
        //deductions
        let quoteProductDeductionList = [...this.props.quoteItemProductDeductionsList]
    ...
    refreshQuoteFinancialCase = async (totalPrice) => {
        let quoteItemFinancialCaseList = [...this.props.quoteItemFinancialCaseList]
        quoteItemFinancialCaseList.map((item) => {
        ...
        await this.props.editQuoteFinancialCaseType(quoteItemFinancialCaseList)
    }
    /* #endregion */
    ...
    deleteProductFinancialCase = () => {
        this.state.deletedProductList.map(y => {
            let additionList = this.props.quoteItemProductAdditionsList.filter(x => x.productRef !== y.id)
            let additionData = {
                tempList: additionList,
                sumAllAdditions: this.props.sumAllAdditions
            }
            this.props.editQuoteProductAdditions(additionData)

            let deductionList = this.props.quoteItemProductDeductionsList.filter(x => x.productRef !== y.id)
            let deductionData = {
                tempList: deductionList,
                sumAllDeductions: this.props.sumAllDeductions
            }
            this.props.editQuoteProductDeductions(deductionData)
    ...
    save = async () => {
        if (this.state.isProductGridHidden === false) {
            ...
            if (this.state.selectedProductList.length === 0) {
                ...
                await this.props.editQuoteProductData(this.state.selectedProductList)
    ...
    delete = async () => {
        ...
        await this.props.editQuoteProductData(list)
    ...
    onCellValueChanged = async (params) => {
        ...
        if (colId === "quantity") {
            ...
            await this.props.editQuoteProductData(selectedProductList);
...
const mapDispatchToProps = dispatch => ({
    ...
    editQuoteProductData: (data) => dispatch(editQuoteProductData(data)),
    ...
    editQuoteFinancialCaseType: (data) => dispatch(editQuoteFinancialCaseType(data)),
