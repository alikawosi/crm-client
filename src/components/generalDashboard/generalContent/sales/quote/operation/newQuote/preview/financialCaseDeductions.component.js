...
/* #endregion */
        ...
        createData = () => {
            var result = [];
            if (Object.keys(this.props.quoteFinancialCaseList).length > 0) {
                let financialCasePrice = 0
                let financialCasePercent = 0
                this.props.quoteFinancialCaseList.filter(y => y.deductionsFlag === true).map(x => {
    ...
    /* #region  [- gridLoadData -] */
    gridLoadData = () => {
        let list = this.props.quoteFinancialCaseList.filter(x => x.deductionsFlag === true)
        this.setState({
            rowData: list
        })
    }
