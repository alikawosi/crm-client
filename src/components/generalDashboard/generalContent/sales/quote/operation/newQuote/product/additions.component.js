...
import { saveQuoteProductAdditions, postQuoteFinancialCaseType, getQuoteFinancialCaseTypeTitle } from '../../../../../../../../redux/sales/quote/quote/quote.action'
...
class Additions extends PureComponent {
    ...
    /* #region  [- gridLoadData -] */
    gridLoadData = () => {
        let list = this.props.quoteProductAdditionList.filter(x => x.productRef === this.props.productRef)
    ...
    submit = () => {
        ...
        let tempList = this.props.quoteProductAdditionList.filter(x => x.productRef !== this.props.productRef)
        this.state.rowData.map(x => {
            tempList.push(x)
        })
