...
import { getQuote, } from '../../../../../../redux/sales/quote/quote/quote.action'
import TimelineCellRenderer from './timelineCellRenderer.component'
import { getSeenQuoteItem, getPrintQuoteItem, saveIsDeleteButonDisabled, saveTimelineIdList, deleteTimeline, } from '../../../../../../redux/sales/quote/timeline/timeline.action'
/* #endregion */
...
class Timeline extends PureComponent {
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("540")) {
            this.setState({
                isNewHidden: false,
            });
        }
        if (data.includes("541")) {
            this.setState({
    ...
    approve = async () => {
        if (this.state.isNewTimelineChecked) {
            this.props.newTimeline(this.state.quoteRef);
        }
    }
    /* #endregion */
    ...
    onCloseDeleteModal = async () => {
        ...
        this.gridApi.setRowData(this.props.quoteList);
        this.setState({
            isDeleteModalVisible: false,
        })
...
const mapStateToProps = state => {
    return {
        quoteList: state.quoteTimeline.quoteList,
        isDeleteButtonDisabled: state.quoteTimeline.isDeleteButtonDisabled,
        timelineIdList: state.quoteTimeline.timelineIdList,
        timelineCellRendererRowId: state.quoteTimeline.timelineCellRendererRowId,
