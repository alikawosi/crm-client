...
class ProductQuantity extends PureComponent {
    ...
    save = async () => {
        ...
        var returnRequestesNumber = selectedData[0].returnRequestesNumber;
        ...
        if (returnRequestesNumber === "" || returnRequestesNumber === undefined || returnRequestesNumber < 1 || isNaN(returnRequestesNumber) === true) {
            InvoiceQuantityNotification('topLeft', 'تعداد بازگشتی نادرست است', 'error');
        }
        else {
            ...
            let isReturnRequestesNumberPermitted = false
            let requestesNumber = 0
            let selectedProductListReturnRequestesNumber = 0

            requestesNumber = this.props.invoiceProductQuantityList.filter(x => x.inventoryId === inventoryId)[0].requestesNumber * 1
          
       
            selectedProductListReturnRequestesNumber = lenSelectedProductList===0 || lenFilteredSelectedProductList===0 ?0:this.state.selectedProductList.filter(y => y.inventoryId === inventoryId)[0].returnRequestesNumber
          
            isReturnRequestesNumberPermitted =  requestesNumber>=returnRequestesNumber+selectedProductListReturnRequestesNumber   ? true : false

            if (isReturnRequestesNumberPermitted === false) {
                InvoiceQuantityNotification('topLeft', 'تعداد بازگشتی از مقدار قابل بازگشت بیشتر است.', 'error');
            }
            else {
                if (doesRowExist > 0) {
                    selectedData[0].returnRequestesNumber = returnRequestesNumber + selectedProductListReturnRequestesNumber
                ...
                if (Object.keys(newList).length > 0) {
                    ...
                    newList.forEach(element => {
                        finalList.push({
                            ...
                            returnRequestesNumber: element.returnRequestesNumber,
                            addInventoryFlag:element.addInventoryFlag,
                    ...
                    await this.gridApi.setRowData(this.props.invoiceProductQuantityList);
    ...
    submit = async () => {
        ...
        let finalFumAllProductQuantity = this.props.sumAllProductQuantity.filter(x => x.productRef !== this.props.productRef)
