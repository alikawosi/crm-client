...
class GridQuantityButton extends PureComponent {
    ...
    /* #region  [- gridLoadData -] */
    gridLoadData = () => {
        if (this.props.sumProductQuantity.length !== 0) {
            let sumAllSelectedProductQuantity = this.props.sumProductQuantity.filter(y => y.productRef ===this.props.node.data.id)
...
const mapStateToProps = state => {
    return {
        sumProductQuantity: state.salesReturn.sumProductQuantity,
        invoiceItemProductQuantityList: state.salesReturn.invoiceItemProductQuantityList,
    }
};
/* #endregion */
