    ...
    /* #endregion */
     ...
     gridLoadData = () => {
        if (this.props.sumAllDeductions.length !== 0) {
            ...
            let list = this.props.invoiceItemProductDeductionsList.filter(x => x.productRef === this.props.node.data.id)
            let sumDeduction = 0
            list.map(x => {
            ...
            let rowCount = this.props.invoiceItemProductDeductionsList.filter(x => x.productRef === this.props.node.data.id).length
...
const mapStateToProps = state => {
    return {
        ...
        invoiceItemProductDeductionsList: state.salesReturn.invoiceItemProductDeductionsList,
    }
};
/* #endregion */
