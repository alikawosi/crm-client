...
import { getSeenInvoiceItem, getPrintInvoiceItem, getInvoice, } from '../../../../../../redux/sales/invoice/invoice/invoice.action'
import TimelineCellRenderer from './timelineCellRenderer.component'
import { saveIsDeleteButonDisabled, saveTimelineIdList, deleteTimeline, } from '../../../../../../redux/sales/invoice/timeline/timeline.action'
/* #endregion */
...
isDeleteHidden:true,
  ...
  /* #endregion */
    ...
    /* #endregion */
        ...
        /* #region  [- accessToMenu -] */
        accessToMenu = (data) => {
            if (data.includes("642")) {
              this.setState({
                isNewHidden: false,
              });
            }
            if (data.includes("643")) {
                this.setState({
    ...
    approve = async () => {
        if (this.state.isNewTimelineChecked) {
            this.props.newTimeline(this.state.invoiceRef);
        }
    }
    /* #endregion */
    ...
    onCloseDeleteModal = async () => {
        ...
        this.gridApi.setRowData(this.props.invoiceList);
        this.setState({
            isDeleteModalVisible: false,
        })
...
const mapStateToProps = state => {
    return {
        invoiceList: state.invoiceTimeline.invoiceList,
        isDeleteButtonDisabled: state.invoiceTimeline.isDeleteButtonDisabled,
        timelineIdList: state.invoiceTimeline.timelineIdList,
        timelineCellRendererRowId: state.invoiceTimeline.timelineCellRendererRowId,
