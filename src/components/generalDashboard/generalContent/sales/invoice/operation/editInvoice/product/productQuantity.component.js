...
class ProductQuantity extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isProductRequestsNumberMessageVisible: false,
            ...
            selectedInventoryColumnDefs: [
                ...
                { headerName: 'تعداد', field: "requestesNumber", colId: "invoicePequestesNumber", valueFormatter: this.currencyFormatter, pinnedRowCellRenderer: 'customPinnedRowRenderer', },
    ...
    save = async () => {
        ...
        var requestesNumber = selectedData[0].requestesNumber;
        var productRef = selectedData[0].productRef;
        var inventoryId = selectedData[0].inventoryId;
        ...
        if (requestesNumber === "" || requestesNumber === undefined || requestesNumber < 1 || isNaN(requestesNumber) === true) {
            ...
            let lenSelectedProductList=Object.keys(this.state.selectedProductList).length
            let lenFilteredSelectedProductList=Object.keys(this.state.selectedProductList.filter(y => y.inventoryId === inventoryId)).length
            let lenPureInvoiceItemProductQuantityList=Object.keys(this.props.pureInvoiceItemProductQuantityList.filter(y => y.inventoryId === inventoryId && y.productRef===productRef)).length

            let newList = []
            let doesRowExist = Object.keys(this.state.selectedProductList.filter(y => y.inventoryId === inventoryId)).length


            let isInventoryRequestesNumberPermitted = false
            let productInventory = 0
            let pureInvoiceItemProductRequestesNumber = 0
            let selectedProductListRequestesNumber = 0

            productInventory = this.props.invoiceInventoryList.filter(x => x.inventoryId === inventoryId)[0].productInventory * 1
          
            pureInvoiceItemProductRequestesNumber = lenPureInvoiceItemProductQuantityList===0?0:this.props.pureInvoiceItemProductQuantityList.filter(y => y.inventoryId === inventoryId && y.productRef===productRef)[0].requestesNumber * 1
           
            selectedProductListRequestesNumber = lenSelectedProductList===0 || lenFilteredSelectedProductList===0 ?0:this.state.selectedProductList.filter(y => y.inventoryId === inventoryId)[0].requestesNumber
          
            isInventoryRequestesNumberPermitted = productInventory + pureInvoiceItemProductRequestesNumber >= requestesNumber + selectedProductListRequestesNumber ? true : false

            if (isInventoryRequestesNumberPermitted === false) {
                InvoiceQuantityNotification('topLeft', 'تعداد درخواستی از مقدار قابل سفارش بیشتر است.', 'error');
            }
            else {
                if (doesRowExist > 0) {
                    selectedData[0].requestesNumber = requestesNumber + selectedProductListRequestesNumber
                    newList.push(selectedData[0])
                }
                else if (doesRowExist === 0) {
                    newList.push(selectedData[0])
                }
                if (Object.keys(newList).length > 0) {
                    let finalList = this.state.selectedProductList.filter(x => x.inventoryId !== inventoryId)
                    newList.forEach(element => {
                        finalList.push({
                            inventoryId: element.inventoryId,
                            productRef: element.productRef,
                            warehouseTitle: element.warehouseTitle,
                            requestesNumber: element.requestesNumber,
                    ...
                    await this.gridApi.setRowData(this.props.invoiceInventoryList);
    ...
    /* #region  [- onOkProductRequestsNumberMessages -] */
    onOkProductRequestsNumberMessages = async () => {
    ...
    /* #region  [- onCancelProductRequestsNumberMessages -] */
    onCancelProductRequestsNumberMessages = () => {
        this.setState({
            isProductRequestsNumberMessageVisible: false,
            isSaveDisabled: true,
            modalComponent: <div></div>
    ...
    /* #region  [- submit -] */
    submit = async () => {
        let tempList = this.props.invoiceItemProductQuantityList.filter(x => x.productRef !== this.props.productRef)
        this.state.selectedProductList.map(x => {
            tempList.push(x)
        })
        ...
        await this.props.editProductTotalQuantity(data)
        this.cancel()
    }
    /* #endregion */
    ...
    onCellValueChanged = async (params) => {
        ...
        if (colId === "requestesNumber") {
            this.props.invoiceInventoryList.filter(x => x.inventoryId * 1 === params.data.inventoryId * 1)[0].requestesNumber = isNaN(params.data.requestesNumber) ? '' : params.data.requestesNumber * 1;
        }
        if (colId === "descriptionRow") {
            this.props.invoiceInventoryList.filter(x => x.inventoryId * 1 === params.data.inventoryId * 1)[0].descriptionRow = params.data.descriptionRow
        }
        ...
        return (
            <Container fluid style={{ padding: '0 2%' }}>
                ...
                <Row name='row_07_Modals'>
                    ...
                    <Modal name="show ProductRequestsNumberMessages"
                        closable={true}
                        maskClosable={false}
                        width='600px'
                        bodyStyle={{ padding: '0px' }}
                        visible={this.state.isProductRequestsNumberMessageVisible}
                        onOk={this.onOkProductRequestsNumberMessages}
                        onCancel={this.onCancelProductRequestsNumberMessages}
...
const mapStateToProps = state => {
    return {
        ...
        pureInvoiceItemProductQuantityList: state.invoice.pureInvoiceItemProductQuantityList,
