...
class GridAdditionsButton extends PureComponent {
    ...
    gridLoadData = () => {
        if (this.props.sumAllAdditions.length !== 0) {
            ...
            let list = this.props.invoiceItemProductAdditionsList.filter(x => x.productRef === this.props.node.data.id)
            let sumAddtion = 0
            list.map(x => {
            ...
            //let rowCount = this.props.invoiceItemProductAdditionsList.filter(x => x.productRef === this.props.node.data.id).length
