...
import {
    getInvoicePriceListProduct, getInvoiceProduct, editInvoiceProductData, saveProductTotalPrice,
    editInvoiceProductAdditions, editInvoiceProductDeductions, editInvoiceFinancialCaseType, getInvoiceInventory,
    setSaveButtonDisabled,
    saveInvoiceRegisteredDocumentsProductData, editProductTotalQuantity,
...
class Product extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedProductColumnDefs: [
                ...
                { headerName: 'مبلغ پیش فرض', field: "unitPrice", width: 80, valueFormatter: this.currencyFormatter, headerComponentParams: { menuIcon: 'fa fa-pencil' }, editable: true, colId: "unitPrice", cellRenderer: this.unitPriceValueGetter, pinnedRowCellRenderer: 'customPinnedRowRenderer', },
                {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.invoiceItemProductList !== this.props.invoiceItemProductList) {
    ...
    productGridLoadData = async () => {
        ...
        this.setState({
            selectedProductList: this.props.invoiceItemProductList
        })
    ...
    onCellValueChanged = async (params) => {
        ...
        if (colId === "unitPrice") {
            ...
            await this.props.editInvoiceProductData(selectedProductList);
    ...
    refreshInvoiceProductAdditionsDeductions = async (selectedProductList, productRef) => {
        ...
        //additions
        let invoiceProductAdditionList = [...this.props.invoiceItemProductAdditionsList]
        ...
        if (additionsLen > 0) {
            ...
            this.props.editInvoiceProductAdditions(data);
        }
        //deductions
        let invoiceProductDeductionList = [...this.props.invoiceItemProductDeductionsList]
        ...
        if (decductionsLen > 0) {
            ...
            this.props.editInvoiceProductDeductions(data);
        }
        await this.calculatePrice(selectedProductList);
    }
    /* #endregion */
    ...
    refreshInvoiceFinancialCase = async (totalPrice) => {
        let invoiceItemFinancialCaseList = [...this.props.invoiceItemFinancialCaseList]
        invoiceItemFinancialCaseList.map((item) => {
        ...
        await this.props.editInvoiceFinancialCaseType(invoiceItemFinancialCaseList)
    }
    /* #endregion */
    ...
    deleteProductFinancialCase = () => {
        this.state.deletedProductList.map(y => {
            let additionList = this.props.invoiceItemProductAdditionsList.filter(x => x.productRef !== y.id)
            let additionData = {
                tempList: additionList,
                sumAllAdditions: this.props.sumAllAdditions
            }
            this.props.editInvoiceProductAdditions(additionData)

            let deductionList = this.props.invoiceItemProductDeductionsList.filter(x => x.productRef !== y.id)
            let deductionData = {
                tempList: deductionList,
                sumAllDeductions: this.props.sumAllDeductions
            }
            this.props.editInvoiceProductDeductions(deductionData)
    ...
    calculateInvoiceProductQuantity = async () => {
        let tempList = this.props.invoiceItemProductQuantityList
    ...
    save = async () => {
        if (this.state.isProductGridHidden === false) {
            ...
            if (this.state.selectedProductList.length === 0) {
                ...
                await this.props.editInvoiceProductData(this.state.selectedProductList)
                this.setState({
                    productList: [],
                    registeredDocumentsList: [],
                    priceListRef: '',
    ...
    delete = async () => {
        ...
        await this.props.editInvoiceProductData(list)
...
const mapDispatchToProps = dispatch => ({
    ...
    editInvoiceProductData: (data) => dispatch(editInvoiceProductData(data)),
