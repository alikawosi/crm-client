...
import { postQuoteToInvoiceCorresponding, getInvoiceInventory } from '../../../../../../../../../../redux/sales/invoice/invoice/invoice.action'
...
class Corresponding extends PureComponent {
    ...
    validateFormOnButtonClick = () => {
        let result = true
        let quoteProductList = [...this.props.quoteProductList]
        for (let i = 0; i < Object.keys(quoteProductList).length; i++) {
            let filteredList = this.props.invoiceProductQuantityList.filter(x => x.productRef === quoteProductList[i].id)
    ...
    cancel = async () => {
        ...
        this.props.onCloseQuoteToInvoiceModal();
    ...
    submit = async () => {
        ...
        if (this.validateFormOnButtonClick() === true) {
            await this.postQuoteToInvoiceCorresponding();
            await this.props.onCloseQuoteToInvoiceModal();
    ...
    onCancelProductQuantity = async (productRef) => {
        ...
        let list = [...this.props.quoteProductList]
    ...
    postQuoteToInvoiceCorresponding = async () => {
        ...
        await this.props.postQuoteToInvoiceCorresponding(JSON.stringify(data))
    }
    /* #endregion */
...
const mapStateToProps = state => {
    return {
        ...
        quoteProductList: state.invoice.quoteProductList,
...
const mapDispatchToProps = dispatch => ({
    ...
    postQuoteToInvoiceCorresponding: (data) => dispatch(postQuoteToInvoiceCorresponding(data)),
