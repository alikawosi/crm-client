...
import { postOrderToInvoiceSplit, saveOrderToInvoiceSplitedData } from '../../../../../../../../../../redux/sales/invoice/invoice/invoice.action'
...
class Split extends PureComponent {
            ...
            detailCellRendererParams: {
                ...
                detailGridOptions: {
                    columnDefs: [
                        ...
                        { headerName: 'تامین کننده', field: "supplyChainTitle", },
                        { headerName: 'نام گروه کالا', field: "productCategoryTitle", },
                        { headerName: 'مبلغ فرض', field: "unitPrice", valueFormatter: this.currencyFormatter, },
                        { headerName: 'تعداد', field: "quantity", valueFormatter: this.currencyFormatter, },
                        { headerName: 'مبلغ', field: "price", valueFormatter: this.currencyFormatter, },
                        { headerName: 'اضافات', field: "invoiceAdditions", valueFormatter: this.currencyFormatter, },
                        { headerName: 'کسورات', field: "invoiceDeductions", valueFormatter: this.currencyFormatter, },
                        { headerName: 'مبلغ نهایی', field: "finalPrice", valueFormatter: this.currencyFormatter, },
                        { headerName: 'توضیحات', field: "descriptionRow" }
                    ],
    ...
    getDetailData = (params) => {
        let list = this.props.invoiceSplitProductList
        params.successCallback(list.filter(x => x.masterCode === params.data.code))
        this.onPinnedRowBottomCountInvoice(params, list);
    }
    /* #endregion */

    /* #region  [- onPinnedRowBottomCountInvoice -] */
    onPinnedRowBottomCountInvoice = (params, list) => {
        var rows = this.createDataInvoice(list);
        let detailGridApi = this.invoiceGridApi.getDetailGridInfo(params.node.id)
        detailGridApi.api.setPinnedBottomRowData(rows);
    }
    /* #endregion */

    /* #region  [- createDataInvoice -] */
    createDataInvoice = (list) => {
        var result = [];
        let unitPrice = 0
        let quantity = 0
        let price = 0
    ...
    resetForm = () => {
        ...
        this.props.invoiceSplitProductList.map(x => {
    ...
    save = async () => {
        ...
        let newProductList = [...this.props.invoiceSplitProductList]
        for (let i = 0; i < Object.keys(selectedData).length; i++) {
            let data = {
                ...
                invoiceAdditions: selectedData[i].orderAdditions,
                invoiceDeductions: selectedData[i].orderDeductions,
        ...
        let invoiceHeaderList = [...this.props.invoiceSplitList]
        ...
        let data = {
            invoiceSplitList: invoiceHeaderList,
            productList: newProductList
        }

        await this.props.saveOrderToInvoiceSplitedData(data)
    ...
    delete = async () => {
        ...
        let data = {
            invoiceSplitList: invoiceHeaderList.filter(x => x.code !== this.state.rowCode),
    ...
    submit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        if (Object.keys(this.props.invoiceSplitList).length > 0 && Object.keys(this.state.productList).length === 0) {
            await this.postOrderToInvoiceSplit();
    ...
    /* #region  [- onSelectionChangedInvoice -] */
    onSelectionChangedInvoice = () => {
    ...
    postOrderToInvoiceSplit = async () => {
        let data = {
            domainRef: this.props.domain,
            aspNetUsersRef: this.props.userId,
            headerRef: this.props.headerRef,
            invoiceHeaderList: this.props.invoiceSplitList,
            productList: this.props.invoiceSplitProductList,
        }
        await this.props.postOrderToInvoiceSplit(JSON.stringify(data))
    }
    /* #endregion */
        ...
        return (
            <Container fluid style={{ margin: '0', padding: '0' }}>
                ...
                <Row name='row_05_GridTitle' style={{ marginTop: '2%', paddingRight: '1%' }}>
                    <Col sm='12' md='12' lg='12'>
                        <span style={{ fontSize: '15px', fontWeight: '500' }}>فاکتور جدید</span>
                    </Col>

                </Row>

                <Row name='row_06_invoiceSplitGrid'>
                    <Col className="ag-theme-alpine" style={{ width: '100%', padding: '2%', height: '400px' }}>
                        ...
                        <AgGridReact
                            ...
                            masterDetail={true}
                            defaultColDef={this.state.defaultColDef}
                            columnDefs={this.state.invoiceColumnDefs}
                            onGridReady={this.onGridReadyInvoice}
                            rowSelection="single"
                            detailRowHeight='380'
                            rowData={this.props.invoiceSplitList}
                            detailCellRendererParams={this.state.detailCellRendererParams}
                            onSelectionChanged={this.onSelectionChangedInvoice}
                            localeText={AG_GRID_LOCALE_FA}
                        />
...
const mapStateToProps = state => {
    return {
        ...
        invoiceSplitList: state.invoice.invoiceSplitList,
        invoiceSplitProductList: state.invoice.invoiceSplitProductList
    }
};
/* #endregion */
...
const mapDispatchToProps = dispatch => ({
    ...
    postOrderToInvoiceSplit: (data) => dispatch(postOrderToInvoiceSplit(data)),
    saveOrderToInvoiceSplitedData: (data) => dispatch(saveOrderToInvoiceSplitedData(data)),
});
/* #endregion */
