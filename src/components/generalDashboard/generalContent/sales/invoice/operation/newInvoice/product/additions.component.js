...
import { saveInvoiceProductAdditions, postInvoiceFinancialCaseType, getInvoiceFinancialCaseTypeTitle } from '../../../../../../../../redux/sales/invoice/invoice/invoice.action'
...
class Additions extends PureComponent {
    ...
    /* #region  [- gridLoadData -] */
    gridLoadData = () => {
        let list = this.props.invoiceProductAdditionList.filter(x => x.productRef === this.props.productRef)
    ...
    submit = () => {
        ...
        let tempList = this.props.invoiceProductAdditionList.filter(x => x.productRef !== this.props.productRef)
        this.state.rowData.map(x => {
            tempList.push(x)
        })
