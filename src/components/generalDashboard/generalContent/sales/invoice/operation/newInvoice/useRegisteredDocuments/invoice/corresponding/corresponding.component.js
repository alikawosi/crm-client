...
import { postInvoiceToInvoiceCorresponding, getInvoiceInventory } from '../../../../../../../../../../redux/sales/invoice/invoice/invoice.action'
...
class Corresponding extends PureComponent {
    ...
    validateFormOnButtonClick = () => {
        let result = true
        let invoiceProductList = [...this.props.invoiceProductList]
        for (let i = 0; i < Object.keys(invoiceProductList).length; i++) {
            let filteredList = this.props.invoiceProductQuantityList.filter(x => x.productRef === invoiceProductList[i].id)
    ...
    cancel = async () => {
        ...
        this.props.onCloseInvoiceToInvoiceModal();
    ...
    submit = async () => {
        ...
        if (this.validateFormOnButtonClick() === true) {
            await this.postInvoiceToInvoiceCorresponding();
            await this.props.onCloseInvoiceToInvoiceModal();
    ...
    onCancelProductQuantity = async (productRef) => {
        ...
        let quantity = 0
        this.props.invoiceProductQuantityList.filter(z => z.productRef === productRef).map(y => quantity = quantity + y.requestesNumber)
        let list = [...this.props.invoiceProductList]
        list.filter(x => x.id === productRef).map(y => y.quantity = quantity)
    ...
    postInvoiceToInvoiceCorresponding = async () => {
        ...
        await this.props.postInvoiceToInvoiceCorresponding(JSON.stringify(data))
    }
    /* #endregion */
...
const mapDispatchToProps = dispatch => ({
    ...
    postInvoiceToInvoiceCorresponding: (data) => dispatch(postInvoiceToInvoiceCorresponding(data)),
