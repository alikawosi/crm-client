...
class GridQuantityButton extends PureComponent {
    ...
    gridLoadData = () => {
        if (this.props.sumAllProductQuantity.length !== 0) {
            ...
            this.setState({
                sumQuantities: this.props.invoiceProductList.filter(x => x.id === this.props.node.data.id)[0].quantity,
    ...
    countQuantity = () => {
        let data = {
            ...
            quantity: this.props.invoiceProductList.filter(x => x.id === this.props.node.data.id)[0].quantity,
