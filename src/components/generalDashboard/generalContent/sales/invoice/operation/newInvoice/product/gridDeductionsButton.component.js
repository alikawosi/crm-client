...
class GridDeductionsButton extends PureComponent {
    ...
    gridLoadData = () => {
        if (this.props.sumAllDeductions.length !== 0) {
            ...
            let list = this.props.invoiceProductDeductionList.filter(x => x.productRef === this.props.node.data.id)
            let sumDeduction = 0
            list.map(x => {
            ...
            let rowCount = this.props.invoiceProductDeductionList.filter(x => x.productRef === this.props.node.data.id).length
