...
import { resetProps, getTermType, deleteTermType, postTermType, getTermTypeItem } from '../../../../../../redux/sales/termType/termType.action'
import NewTermType from './newTermType.component'
import EditTermType from './editTermType.component'
/* #endregion */

class TermType extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columnDefs: [
                ...
                { headerName: 'حمل و نقل و ترابری', field: 'shippingFlag', },
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("243")) {
            this.setState({
                isNewButtonHidden: false
            })
        }
        if (data.includes("247")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("245")) {
            this.setState({
        ...
        if (data.includes("457")) {
            this.setState({
                isPrintButtonHidden: false
            })
        }
        if (data.includes("458")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.termTypeList
        })
    }
    /* #endregion */
    ...
    getTermType = async () => {
        ...
        await this.props.getTermType(JSON.stringify(data));
    }
    /* #endregion */

    /* #region  [- getTermTypeItem -] */
    getTermTypeItem = async () => {
        let data = {
            id: this.state.id
        }
        await this.props.getTermTypeItem(JSON.stringify(data));
    }
    /* #endregion */

    /* #region  [- deleteTermType -] */
    deleteTermType = async () => {
        let termTypeDeleteData = {
            domainRef: this.props.domain,
            termTypeIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deleteTermType(JSON.stringify(termTypeDeleteData));
    }
    /* #endregion */
    ...
    postExcelFile = async (excelData) => {
        ...
        let termTypePostData = {
            domainRef: this.props.domain,
            termTypeList: excelData
        }
        await this.props.postTermType(JSON.stringify(termTypePostData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewTermType onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getTermTypeItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditTermType onClose={this.onCloseEditDrawer} />
        });
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                        ...
                        <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                            ...
                            <Col className='content-header-col'>
                                <p className='content-header-title'>انواع شرایط و ملاحضات</p>
                            </Col>
                        ...
                        <Row name='row_04_DrawersAndModals'>
                            ...
                            >
                                <Container fluid>
                                    ...
                                    <Row name='content'>
                                        <Col sm='12' >
                                            <ExcelImport name='termType' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                        </Col>
                                    </Row>
                                </Container>
                            </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.termType.message,
        termTypeList: state.termType.termTypeList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getTermType: (data) => dispatch(getTermType(data)),
    deleteTermType: (data) => dispatch(deleteTermType(data)),
    postTermType: (data) => dispatch(postTermType(data)),
    getTermTypeItem: (data) => dispatch(getTermTypeItem(data)),
})
/* #endregion */

export default connect(mapStateToProps, mapDispatchToProps)(TermType);
