...
import { resetProps, getDeliveryTerm, deleteDeliveryTerm, postDeliveryTerm, getDeliveryTermItem } from '../../../../../../redux/sales/deliveryTerm/deliveryTerm.action'
import NewDeliveryTerm from './newDeliveryTerm.component'
import EditDeliveryTerm from './editDeliveryTerm.component'
/* #endregion */

class DeliveryTerm extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columnDefs: [
                ...
                { headerName: 'مخفف', field: 'abbreviation', width: 150 },
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("261")) {
            this.setState({
                isNewButtonHidden: false
            })
        }
        if (data.includes("265")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("263")) {
            this.setState({
        ...
        if (data.includes("461")) {
            this.setState({
                isPrintButtonHidden: false
            })
        }
        if (data.includes("462")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.deliveryTermList
        })
    }
    /* #endregion */
    ...
    getDeliveryTerm = async () => {
        let data = {
            domainRef: parseInt(this.props.domain)
        }
        await this.props.getDeliveryTerm(JSON.stringify(data));
    }
    /* #endregion */

    /* #region  [- getDeliveryTermItem -] */
    getDeliveryTermItem = async () => {
        let data = {
            id: this.state.id
        }
        await this.props.getDeliveryTermItem(JSON.stringify(data));
    }
    /* #endregion */

    /* #region  [- deleteDeliveryTerm -] */
    deleteDeliveryTerm = async () => {
        let deliveryTermDeleteData = {
            domainRef: parseInt(this.props.domain),
            deliveryTermIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deleteDeliveryTerm(JSON.stringify(deliveryTermDeleteData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewDeliveryTerm onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getDeliveryTermItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditDeliveryTerm onClose={this.onCloseEditDrawer} />
        });
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                ...
                <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                    ...
                    <Col className='content-header-col'>
                        <p className='content-header-title'>شرایط تحویل</p>
                    </Col>
                ...
                <Row name='row_04_DrawersAndModals'>
                    ...
                    >
                        <Container fluid>
                            ...
                            <Row name='content'>
                                <Col sm='12' >
                                    <ExcelImport name='deliveryTerm' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                </Col>
                            </Row>
                        </Container>
                    </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.deliveryTerm.message,
        deliveryTermList: state.deliveryTerm.deliveryTermList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getDeliveryTerm: (data) => dispatch(getDeliveryTerm(data)),
    deleteDeliveryTerm: (data) => dispatch(deleteDeliveryTerm(data)),
    postDeliveryTerm: (data) => dispatch(postDeliveryTerm(data)),
    getDeliveryTermItem: (data) => dispatch(getDeliveryTermItem(data)),
})
/* #endregion */
