...
import { resetProps, getFinancialCaseType, deleteFinancialCaseType, postFinancialCaseType, getFinancialCaseTypeItem } from '../../../../../../redux/sales/financialCaseType/financialCaseType.action'
import NewFinancialCaseType from './newFinancialCaseType.component'
import EditFinancialCaseType from './editFinancialCaseType.component'
/* #endregion */

class FinancialCaseType extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columnDefs: [
                ...
                { headerName: 'اضافات/کسورات', field: 'financialCaseFlag',},
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("234")) {
            this.setState({
                isNewButtonHidden: false
            })
        }
        if (data.includes("238")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("236")) {
            this.setState({
        ...
        if (data.includes("455")) {
            this.setState({
                isPrintButtonHidden: false
            })
        }
        if (data.includes("456")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.financialCaseTypeList
        })
    }
    /* #endregion */
    ...
    getFinancialCaseType = async () => {
        let financialCaseTypeData = {
            domainRef: this.props.domain
        }
        await this.props.getFinancialCaseType(JSON.stringify(financialCaseTypeData));
    }
    /* #endregion */

    /* #region  [- getFinancialCaseTypeItem -] */
    getFinancialCaseTypeItem = async () => {
        let financialCaseTypeItemData = {
            id: this.state.id
        }
        await this.props.getFinancialCaseTypeItem(JSON.stringify(financialCaseTypeItemData));
    }
    /* #endregion */

    /* #region  [- deleteFinancialCaseType -] */
    deleteFinancialCaseType = async () => {
        let financialCaseTypeDeleteData = {
            domainRef: this.props.domain,
            financialCaseTypeIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deleteFinancialCaseType(JSON.stringify(financialCaseTypeDeleteData));
    }
    /* #endregion */
    ...
    postExcelFile = async (excelData) => {
        ...
        let financialCaseTypePostData = {
            domainRef: this.props.domain,
            financialCaseTypeList: excelData
        }
        await this.props.postFinancialCaseType(JSON.stringify(financialCaseTypePostData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewFinancialCaseType onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getFinancialCaseTypeItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditFinancialCaseType onClose={this.onCloseEditDrawer} />
        });
    ...
    deleteInModal = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.deleteFinancialCaseType();
    ...
    refresh = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getFinancialCaseType();
        this.reset();
    }
    /* #endregion */
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                        ...
                        <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                            ...
                            <Col className='content-header-col'>
                                <p className='content-header-title'>انواع اضافات و کسورات</p>
                            </Col>
                        ...
                        <Row name='row_04_DrawersAndModals'>
                            ...
                            >
                                <Container fluid>
                                    ...
                                    <Row name='content'>
                                        <Col sm='12' >
                                            <ExcelImport name='financialCaseType' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                        </Col>
                                    </Row>
                                </Container>
                            </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.financialCaseType.message,
        financialCaseTypeList: state.financialCaseType.financialCaseTypeList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getFinancialCaseType: (data) => dispatch(getFinancialCaseType(data)),
    deleteFinancialCaseType: (data) => dispatch(deleteFinancialCaseType(data)),
    postFinancialCaseType: (data) => dispatch(postFinancialCaseType(data)),
    getFinancialCaseTypeItem: (data) => dispatch(getFinancialCaseTypeItem(data)),
})
/* #endregion */

export default connect(mapStateToProps, mapDispatchToProps)(FinancialCaseType);
