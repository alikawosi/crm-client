...
import { resetProps, getPaymentType, deletePaymentType, postPaymentType, getPaymentTypeItem } from '../../../../../../redux/sales/paymentType/paymentType.action'
import NewPaymentType from './newPaymentType.component'
import EditPaymentType from './editPaymentType.component'
/* #endregion */
...
class PaymentType extends PureComponent {
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("252")) {
            this.setState({
                isNewButtonHidden: false
            })
        }
        if (data.includes("256")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("254")) {
            this.setState({
        ...
        if (data.includes("459")) {
            this.setState({
                isPrintButtonHidden: false
            })
        }
        if (data.includes("460")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.paymentTypeList
        })
    }
    /* #endregion */
    ...
    getPaymentType = async () => {
        ...
        await this.props.getPaymentType(JSON.stringify(paymentTypeData));
    }
    /* #endregion */

    /* #region  [- getPaymentTypeItem -] */
    getPaymentTypeItem = async () => {
        let paymentTypeItemData = {
            id: this.state.id
        }
        await this.props.getPaymentTypeItem(JSON.stringify(paymentTypeItemData));
    }
    /* #endregion */

    /* #region  [- deletePaymentType -] */
    deletePaymentType = async () => {
        let paymentTypeDeleteData = {
            domainRef: this.props.domain,
            paymentTypeIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deletePaymentType(JSON.stringify(paymentTypeDeleteData));
    }
    /* #endregion */
    ...
    postExcelFile = async (excelData) => {
        ...
        let paymentTypePostData = {
            domainRef: this.props.domain,
            paymentTypeList: excelData
        }
        await this.props.postPaymentType(JSON.stringify(paymentTypePostData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewPaymentType onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getPaymentTypeItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditPaymentType onClose={this.onCloseEditDrawer} />
        });
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                        ...
                        <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                            ...
                            <Col className='content-header-col'>
                                <p className='content-header-title'>انواع روش پرداخت</p>
                            </Col>
                        ...
                        <Row name='row_04_DrawersAndModals'>
                            ...
                            >
                                <Container fluid>
                                    ...
                                    <Row name='content'>
                                        <Col sm='12' >
                                            <ExcelImport name='paymentType' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                        </Col>
                                    </Row>
                                </Container>
                            </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.paymentType.message,
        paymentTypeList: state.paymentType.paymentTypeList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getPaymentType: (data) => dispatch(getPaymentType(data)),
    deletePaymentType: (data) => dispatch(deletePaymentType(data)),
    postPaymentType: (data) => dispatch(postPaymentType(data)),
    getPaymentTypeItem: (data) => dispatch(getPaymentTypeItem(data)),
})
/* #endregion */
