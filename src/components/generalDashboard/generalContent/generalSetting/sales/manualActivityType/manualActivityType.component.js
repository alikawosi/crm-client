...
import { resetProps, getManualActivityType, deleteManualActivityType, postManualActivityType, getManualActivityTypeItem } from '../../../../../../redux/sales/manualActivityType/manualActivityType.action'
import NewManualActivityType from './newManualActivityType.component'
import EditManualActivityType from './editManualActivityType.component.js'
/* #endregion */

class ManualActivityType extends PureComponent {
    ...
    accessToMenu = (data) => {
        ...
        if (data.includes("722")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("725")) {
            this.setState({
        ...
        if (data.includes("727")) {
            this.setState({
                isPrintButtonHidden: false
            })
        }
        if (data.includes("728")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.manualActivityTypeList
        })
    }
    /* #endregion */
    ...
    getManualActivityType = async () => {
        let manualActivityTypeData = {
            domainRef: this.props.domain
        }
        await this.props.getManualActivityType(JSON.stringify(manualActivityTypeData));
    }
    /* #endregion */

    /* #region  [- getManualActivityTypeItem -] */
    getManualActivityTypeItem = async () => {
        let manualActivityTypeItemData = {
            id: this.state.id
        }
        await this.props.getManualActivityTypeItem(JSON.stringify(manualActivityTypeItemData));
    }
    /* #endregion */

    /* #region  [- deleteManualActivityType -] */
    deleteManualActivityType = async () => {
        let manualActivityTypeDeleteData = {
            domainRef: this.props.domain,
            manualActivityTypeIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deleteManualActivityType(JSON.stringify(manualActivityTypeDeleteData));
    }
    /* #endregion */
    ...
    postExcelFile = async (excelData) => {
        ...
        let manualActivityTypePostData = {
            domainRef: this.props.domain,
            manualActivityTypeList: excelData
        }
        await this.props.postManualActivityType(JSON.stringify(manualActivityTypePostData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewManualActivityType onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getManualActivityTypeItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditManualActivityType onClose={this.onCloseEditDrawer} />
        });
    ...
    deleteInModal = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.deleteManualActivityType();
    ...
    refresh = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getManualActivityType();
        this.reset();
    }
    /* #endregion */
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                        ...
                        <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                            ...
                            <Col className='content-header-col'>
                                <p className='content-header-title'>انواع فعالیت</p>
                            </Col>
                        ...
                        <Row name='row_04_DrawersAndModals'>
                            ...
                            >
                                <Container fluid>
                                    ...
                                    <Row name='content'>
                                        <Col sm='12' >
                                            <ExcelImport name='manualActivityType' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                        </Col>
                                    </Row>
                                </Container>
                            </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.manualActivityType.message,
        manualActivityTypeList: state.manualActivityType.manualActivityTypeList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getManualActivityType: (data) => dispatch(getManualActivityType(data)),
    deleteManualActivityType: (data) => dispatch(deleteManualActivityType(data)),
    postManualActivityType: (data) => dispatch(postManualActivityType(data)),
    getManualActivityTypeItem: (data) => dispatch(getManualActivityTypeItem(data)),
})
/* #endregion */

export default connect(mapStateToProps, mapDispatchToProps)(ManualActivityType);
