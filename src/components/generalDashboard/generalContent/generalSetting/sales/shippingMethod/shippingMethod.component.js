...
import { resetProps, getShippingMethod, deleteShippingMethod, postShippingMethod, getShippingMethodItem } from '../../../../../../redux/sales/shippingMethod/shippingMethod.action'
import NewShippingMethod from './newShippingMethod.component'
import EditShippingMethod from './editShippingMethod.component'
/* #endregion */
...
class ShippingMethod extends PureComponent {
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("271")) {
            this.setState({
                isNewButtonHidden: false
            })
        }
        if (data.includes("276")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("273")) {
            this.setState({
        ...
        if (data.includes("463")) {
            this.setState({
                isPrintButtonHidden: false
            })
        }
        if (data.includes("464")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.shippingMethodList
        })
    }
    /* #endregion */
    ...
    getShippingMethod = async () => {
        ...
        await this.props.getShippingMethod(JSON.stringify(data));
    }
    /* #endregion */

    /* #region  [- getShippingMethodItem -] */
    getShippingMethodItem = async () => {
        let data = {
            id: this.state.id
        }
        await this.props.getShippingMethodItem(JSON.stringify(data));
    }
    /* #endregion */

    /* #region  [- deleteShippingMethod -] */
    deleteShippingMethod = async () => {
        let shippingMethodDeleteData = {
            domainRef: this.props.domain,
            shippingMethodIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deleteShippingMethod(JSON.stringify(shippingMethodDeleteData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewShippingMethod onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getShippingMethodItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditShippingMethod onClose={this.onCloseEditDrawer} />
        });
    ...
    deleteInModal = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.deleteShippingMethod();
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                        ...
                        <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                            ...
                            <Col className='content-header-col'>
                                <p className='content-header-title'>انواع روش حمل و نقل</p>
                            </Col>
                        ...
                        <Row name='row_04_DrawersAndModals'>
                            ...
                            >
                                <Container fluid>
                                    ...
                                    <Row name='content'>
                                        <Col sm='12' >
                                            <ExcelImport name='shippingMethod' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                        </Col>
                                    </Row>
                                </Container>
                            </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.shippingMethod.message,
        shippingMethodList: state.shippingMethod.shippingMethodList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getShippingMethod: (data) => dispatch(getShippingMethod(data)),
    deleteShippingMethod: (data) => dispatch(deleteShippingMethod(data)),
    postShippingMethod: (data) => dispatch(postShippingMethod(data)),
    getShippingMethodItem: (data) => dispatch(getShippingMethodItem(data)),
})
/* #endregion */
