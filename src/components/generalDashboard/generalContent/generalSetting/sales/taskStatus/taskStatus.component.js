...
import { resetProps, getTaskStatus, deleteTaskStatus, postTaskStatus, getTaskStatusItem } from '../../../../../../redux/sales/taskStatus/taskStatus.action'
import NewTaskStatus from './newTaskStatus.component'
import EditTaskStatus from './editTaskStatus.component'
/* #endregion */
...
class TaskStatus extends PureComponent {
    ...
    /* #region  [- accessToMenu -] */
    accessToMenu = (data) => {
        if (data.includes("290")) {
            this.setState({
                isNewButtonHidden: false
            })
        }
        if (data.includes("294")) {
            this.setState({
                isEditButtonHidden: false
            })
        }
        if (data.includes("292")) {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        ...
        this.setState({
            rowData: this.props.taskStatusList
        })
    }
    /* #endregion */
...
/* #region  [- handleChangeDeleteModalInput -] */
    ...
    getTaskStatus = async () => {
        ...
        await this.props.getTaskStatus(JSON.stringify(taskStatusData));
    }
    /* #endregion */

    /* #region  [- getTaskStatusItem -] */
    getTaskStatusItem = async () => {
        let taskStatusItemData = {
            id: this.state.id
        }
        await this.props.getTaskStatusItem(JSON.stringify(taskStatusItemData));
    }
    /* #endregion */

    /* #region  [- deleteTaskStatus -] */
    deleteTaskStatus = async () => {
        let taskStatusDeleteData = {
            domainRef: this.props.domain,
            taskStatusIdList: [{
                id: this.state.id
            }]
        }
        await this.props.deleteTaskStatus(JSON.stringify(taskStatusDeleteData));
    }
    /* #endregion */
    ...
    postExcelFile = async (excelData) => {
        ...
        let taskStatusPostData = {
            domainRef: this.props.domain,
            taskStatusList: excelData
        }
        await this.props.postTaskStatus(JSON.stringify(taskStatusPostData));
    }
    /* #endregion */
    ...
    new = async () => {
        ...
        this.setState({
            ...
            newDrawerContent: <NewTaskStatus onClose={this.onCloseNewDrawer} />
        });
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getTaskStatusItem();
        this.setState({
            isEditDrawerVisible: true,
            editDrawerContent: <EditTaskStatus onClose={this.onCloseEditDrawer} />
        });
            ...
            <Container fluid className='reacstrap-container' style={{ padding: '0' }} >
                ...
                <Row name='row_01_Header' className='content-header-row' style={{ margin: '0 0 1% 0', padding: '0' }}>
                    ...
                    <Col className='content-header-col'>
                        <p className='content-header-title'>وضعیت کار</p>
                    </Col>
                ...
                <Row name='row_04_DrawersAndModals'>
                    ...
                    >
                        <Container fluid>
                            ...
                            <Row name='content'>
                                <Col sm='12' >
                                    <ExcelImport name='taskStatus' sendExcelFileList={this.postExcelFile} columnsDef={this.state.columns} onRef={ref => (this.child = ref)} />
                                </Col>
                            </Row>
                        </Container>
                    </Modal>
...
const mapStateToProps = state => {
    return {
        ...
        message: state.taskStatus.message,
        taskStatusList: state.taskStatus.taskStatusList,
...
const mapDispatchToProps = dispatch => ({
    ...
    getTaskStatus: (data) => dispatch(getTaskStatus(data)),
    deleteTaskStatus: (data) => dispatch(deleteTaskStatus(data)),
    postTaskStatus: (data) => dispatch(postTaskStatus(data)),
    getTaskStatusItem: (data) => dispatch(getTaskStatusItem(data)),
})
/* #endregion */
