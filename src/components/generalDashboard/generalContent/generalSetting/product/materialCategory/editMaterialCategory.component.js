...
import { putMaterialCategory, getMaterialCategoryFullPathById } from '../../../../../../redux/product/materialCategory/materialCategory.action';
import { checkTokenExpiration } from '../../../../../../redux/shared/auth/auth.action';


class EditMaterialCategory extends PureComponent {
    /* #region  [- ctor -] */
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentDidMount() {
        ...
        const materialCategoryItem = Object.assign({}, this.props.materialCategoryItem[0]);
        if (materialCategoryItem.parentId !== null) {
            this.setState({
                id: materialCategoryItem.id,
                upLevel: materialCategoryItem.parentId,
                parentCode: materialCategoryItem.upLevelThreadCode,
                title: materialCategoryItem.title,
                abbreviation: materialCategoryItem.abbreviation,
                code: materialCategoryItem.code,
                separator: materialCategoryItem.separator,
                descriptionRow: materialCategoryItem.descriptionRow,
    ...
    putMaterialCategory = async () => {
        let materialCategoryPutData = {
        ...
        await this.props.putMaterialCategory(materialCategoryPutData);
    }
    /* #endregion */
    ...
    upLevelHandleChange = (event) => {
        ...
        if (event.target.value !== '') {
            let array = this.props.materialCategoryFullPathByIdList;
    ...
    submit = async () => {
        ...
        if (this.validateFormOnButtonClick() === true) {
            await this.putMaterialCategory();
    ...
    render() {
        ...
        const materialCategoryFullPathById = this.props.materialCategoryFullPathByIdList.map(item => (
            ...
            <Container style={{ height: '100vh' }}>
                <Row title='header' className='mb-2' style={{ borderBottom: '1px solid #f0f0f0', height: '5vh' }}>
                    <Col style={{ direction: 'rtl', textAlign: 'right' }}>
                        <span style={{ height: '48px', lineHeight: '48px', fontSize: '17px', fontWeight: 'bold' }}>ویرایش گروه کالا پایه</span>
                    </Col>
                </Row>
...
const mapStateToProps = state => {
    return {
        ...
        materialCategoryFullPathByIdList: state.materialCategory.materialCategoryFullPathByIdList,
        //
...
const mapDispatchToProps = dispatch => ({
    putMaterialCategory: (data) => dispatch(putMaterialCategory(data)),
...
export default connect(mapStateToProps, mapDispatchToProps)(EditMaterialCategory);