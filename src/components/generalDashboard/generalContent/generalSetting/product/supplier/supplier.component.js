...
import NewSupplier from './newSupplier.component';
import EditSupplier from './editSupplier.component';
import './supplier.component.css';
...
class Supplier extends PureComponent {
    ...
    async componentDidMount()  {
        ...
        this.setState({
            supplierList: this.props.supplierList.filter(item =>item.supplierFlag===true)
        })
    ...
    componentDidUpdate(prevProps) {
        if (this.props.message !== prevProps.message) {
         ...
         this.setState({
             rowData: this.state.supplierList
         })
    }
    /* #endregion */
    ...
    getSupplier = async () => {
        ...
        await this.props.getSupplier(supplyChainGetData);
    }
    /* #endregion */
    
    /* #region  [- getSupplierItem() -] */
    
    getSupplierItem = async () => {
    let supplyChainItemGetData = {
      supplyChainId: this.state.id,
    }
    await this.props.getSupplierItem(supplyChainItemGetData);
  }
  /* #endregion */
    ...
    deleteSupplier = async () => {
        ...
        await this.props.deleteSupplier(supplyChainDeleteData);
        await this.getSupplier();
        this.setState({
            supplierList: this.props.supplierList.filter( item => item.supplierFlag===true)
        })
    ...
    new = async () => {
        ...
        this.setState({
            newDrawerContent: <NewSupplier onClose={this.toggleNewDrawer} />
        })
    }
    /* #endregion */
    ...
    edit = async () => {
        await this.props.checkTokenExpiration(this.props.checkTokenCounter);
        await this.getSupplierItem();
        let supplierSelectedItem = { ...this.props.supplierItem[0] }
        if (supplierSelectedItem.id === undefined) {
            await this.getSupplier();
        }
        else {
            await this.toggleEditDrawer();
            this.setState({
                editDrawerContent: <EditSupplier supplierId={this.state.id} onClose={this.toggleEditDrawer} />
            })
        }
    }
    /* #endregion */
...
const mapStateToProps = state => {
    return {
        supplierList: state.supplyChain.supplyChainList,
        supplierItem: state.supplyChain.supplyChainItem,
...
const mapDispatchToProps = dispatch => ({
    ...
    getSupplier: (data) => dispatch(getSupplyChain(data)),
    getSupplierItem: (data) => dispatch(getSupplyChainItem(data)),
    deleteSupplier:(data) => dispatch(deleteSupplyChain(data)),
