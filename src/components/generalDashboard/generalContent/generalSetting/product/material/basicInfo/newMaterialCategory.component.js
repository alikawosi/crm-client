...
class NewMaterialCategory extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isAbbreviationInvalid: false,
            isAbbreviationValid: false,
        ...
        if (this.state.isUpLevelSwitchChecked === false) {
            switch (event.target.id) {
                ...
                case "abbreviation":
                    if (event.target.value === "") {
                        this.setState({
                            isAbbreviationInvalid: true,
                            isAbbreviationValid: false
                        });
                        errors["abbreviation"] = "مخفف اجباری است";
                    }
                    else {
                        this.setState({
                            isAbbreviationInvalid: false,
    ...
    /* #region  [- getMaterialCategoryFullPath -] */
    getMaterialCategoryFullPath = async () => {
        let materalCategoryTitleGetData = {
            domainRef: this.props.domain
        }

        await this.props.getMaterialCategoryFullPath(materalCategoryTitleGetData)
    }
    /* #endregion */
                    ...
                    <Col name='form' sm='12'>
                        <Form>
                            ...
                            <FormGroup name='abbreviation' style={{ textAlign: 'right' }}>
                                <Label for="abbreviation">مخفف<span className="form-mandatory-field-star">*</span></Label>
                                <Input
                                    type="text"
                                    name="abbreviation"
                                    id="abbreviation"
                                    ...
                                    placeholder="مخفف"
                                    onChange={this.handleChange}
                                    invalid={this.state.isAbbreviationInvalid}
                                    valid={this.state.isAbbreviationValid}
                                />
                                <FormFeedback>{this.state.errors.title}</FormFeedback>
                            </FormGroup>
                            ...
                            <Row form style={{ direction: 'rtl' }}>
                                ...
                                <Col name='separator' md={4}>
                                    <FormGroup style={{ textAlign: 'right' }}>
                                        <Label for="separator">جداکننده<span className="form-mandatory-field-star">*</span></Label>
                                        <Input
                                            type="select"
                                            name="separator"
                                            id="separator"
                                ...
                                <Col md={4} style={{ textAlign: 'right' }}>
                                    <FormGroup title='code' style={{ textAlign: 'right' }}>
                                        ...
                                        <Input
                                            ...
                                            onChange={this.handleChange}
                                            invalid={this.state.isCodeInvalid}
                                            valid={this.state.isCodeValid}
                                        />
