...
class ProductAttachFile extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isAttachmentFormHidden:false,
            isAttachFileHeaderHidden:true,
            //fields
            id: 0,
            productRef: null,
            title: '',
            ...
            columnDefs: [
                ...
                { headerName: 'فایل ', field: "attachedFile", cellRenderer:'gridFileDownloadButton' },
            ],
            gridOptions:{
    ...
    componentDidMount() {
        if(this.props.mainGridCalling === true){

            this.setState({
                isAttachmentFormHidden:true,
                isAttachFileHeaderHidden:false
            })
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.productCRMFileList !== prevProps.productCRMFileList) {
            this.setState({
                rowData: this.props.productCRMFileList
            })
        }
    }
    /* #endregion */
    ...
    getProductCRMFile = async () => {
        let productCRMFileGetData = {
             id: this.props.productId
        }
        await this.props.getProductCRMFile(productCRMFileGetData);
    }
    /* #endregion */

    /* #region  [- postProductCRMFile() -] */
    postProductCRMFile = async () => {
        let productCRMFilePostData = {
            productCRMFilePostList: [
                {
        ...
        //console.log('postdata',productCRMFilePostData);
        await this.props.postProductCRMFile(productCRMFilePostData);
    }
    /* #endregion */

    /* #region  [- deleteProductCRMFile() -] */
    deleteProductCRMFile = async () => {
        let productCRMFileDeleteData = {
            productCRMFileIdList: [
                {
                    id: this.state.id
                }
            ]
        }
        await this.props.deleteProductCRMFile(productCRMFileDeleteData);
    }
    /* #endregion */
    ...
    render() {
        ...
        return (
            <Container>
                <Row title='header' className='mt-2' hidden={this.state.isAttachFileHeaderHidden}>
                    <Col style={{ direction: 'rtl', textAlign: 'right' }}>
                        <span style={{ height: '48px', lineHeight: '48px', fontSize: '25px' }}>فایل های پیوست</span>
                    </Col>
                    <hr />
                </Row>

                <Row title='form' hidden={this.state.isAttachmentFormHidden}>
                    <Col sm='12'>
                        <Form>
                ...
                <Row title='buttons' hidden={this.state.isAttachmentFormHidden} >
                ...
                <Row title='grid' style={{marginTop:'20px',marginBottom:'20px'}}>
